...
This core slice does not itself resolve the first iproute2 release containing `ip netns`, first real-world deployment, or the full post-2.6.24 protocol-family completion chronology. The [companion userspace study](iproute2-ip-netns-first-tagged-release.md) independently recovers the first tagged release/source snapshot at v3.0.0, not first public tarball publication or deployment.
...
3. It does not prove that `ip netns` existed in the first iproute2 release contemporaneous with the kernel core merge. The companion userspace evidence instead pins the later named/processless command family to the v3.0.0 tagged snapshot; the stronger first-public-tarball question remains open.
