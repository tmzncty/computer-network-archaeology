...
- [~] network namespace merge series, veth merge provenance, and `ip netns` first-release archaeology. **Completed:** exact initial veth mainline commit `e314dbdc1c0dc6a548ecf0afce28ecfd538ff568`, initial pair/namespace intent, and first stable mainline presence in Linux 2.6.24; the bounded [veth public proposal/revision/acceptance slice](../routing/linux-veth-upstream-provenance.md) now distinguishes June 6 proposal/review, July v2.1 patch and v3/v4 cover reports, August 8 local / August 9 UTC subsystem statement, and the preserved commit's separate author/committer dates (`SRC-0280..0285`, `SRC-0269`). **Still missing:** exact network-namespace subsystem merge series, first iproute2 release containing `ip netns`, complete intermediate veth subpatches and earliest-prototype/first-deployment evidence; specific OpenVZ descent and Linus first-parent integration are not established. Retain the separate netns/ip-netns research when combining contributions; this veth supplement does not close those questions.
